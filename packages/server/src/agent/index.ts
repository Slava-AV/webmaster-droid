...
function toHistoryModelMessages(
  history?: Array<{ role: "user" | "assistant"; text: string }>
): ModelMessage[] {
...
function formatSelectedElementContext(selectedElement?: SelectedElementContext): string {
  if (!selectedElement) {
...
function previewDocument(document: CmsDocument): string {
...
type GenerateImageMode = "new" | "edit";
type GenerateImageQuality = "1K" | "2K" | "4K";

interface GeminiInlineImagePayload {
  mimeType: string;
  data: string;
}
...
function searchDocument(document: CmsDocument, query: string) {
